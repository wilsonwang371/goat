...
type MemBarFeed interface {
	common_old.BarFeed
	AddBarListFromSequence(instrument string, barList []common_old.Bar) error
	LoadAll() error
}
...
type memBarFeed struct {
	baseBarFeed
	started         bool
	bars            common_old.Bars
	nextIdx         map[string]int
	currentDateTime *time.Time
}

func NewMemBarFeed(freqList []common_old.Frequency, sType series.Type, maxLen int) *memBarFeed {
...
func (m *memBarFeed) AddBarListFromSequence(instrument string, barList []common_old.Bar) error {
	if m.started {
	...
	allFreq := map[common_old.Frequency]bool{}
	...
	for freq := range allFreq {
		if err := m.RegisterInstrument(instrument, []common_old.Frequency{freq}); err != nil {
			return err
		}
	}
	return nil
...
func (m *memBarFeed) GetNextBars() (common_old.Bars, error) {
