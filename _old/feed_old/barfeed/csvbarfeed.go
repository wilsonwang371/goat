...
type BarFilter interface {
	IncludeBar(bar common_old.Bar) bool
}
...
func NewCSVBarFeed(freqList []common_old.Frequency, sType series.Type, timezone string, maxLen int) *CSVBarFeed {
	if len(freqList) != 1 {
...
func (c *CSVBarFeed) parseRawToBar(dict map[string]string) (common_old.Bar, error) {
...
func (c *CSVBarFeed) AddBarsFromCSV(instrument string, path string, _ string) error {
	...
	var loadedBarList []common_old.Bar
	for {
