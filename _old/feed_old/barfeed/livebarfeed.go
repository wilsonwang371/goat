...
type LiveBarFeed struct {
	...
	stopC      chan struct{}
	doneC      chan struct{}
	barsBuffer []common_old.Bars
	fetcher    common_old.LiveBarFetcher
}

func NewLiveBarFeed(f common_old.LiveBarFetcher, maxLen int) *LiveBarFeed {
...
func (l *LiveBarFeed) GetNextBars() (common_old.Bars, error) {
	l.mu.Lock()
	defer l.mu.Unlock()
